  ...
  return (
    <aside className="flex w-full max-w-xs flex-col rounded-3xl border border-[var(--color-border-soft)] bg-[var(--color-page-bg)] p-6 shadow-soft lg:sticky lg:top-4 lg:h-[calc(100vh-2rem)]">
      ...
      <div className="mt-5 flex items-center justify-between gap-3">
        ...
        <Button
          className="h-10 w-auto px-3 text-xs"
      ...
      {isCreateMode ? (
        <form
          className="mt-4 space-y-3 rounded-2xl border border-[var(--color-border-soft)] bg-[var(--color-page-bg)] p-4"
        ...
        {folders.map((folder) => {
          ...
          return (
            ...
            >
              ...
              <span className={cn('text-xs', isSelected ? 'text-[var(--color-option-selected-muted)]' : 'text-[var(--color-text-muted)]')}>文件夹</span>
            </button>
          );
        })}
