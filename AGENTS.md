...
- 每次任务完成后，Codex **必须主动告诉 ice 建议的下一步要做什么**，而不是等待 ice 再追问。
...
- 前端基础交互组件默认使用 **shadcn/ui 风格组件基座** 实现，包括但不限于 `Button`、`Input`、`Card`、`Toast`。
- `shadcn/ui` 只负责组件结构、交互状态与可维护性；**视觉主题仍然完全服从本项目自己的 Tailwind + CSS 变量配色方案**。
- 后续如果补充新的基础组件，优先继续沿用 shadcn/ui 风格与 Radix 原语，而不是再写一套完全独立的自定义交互基座。
- 后续如果补充新的**展示层组件**（如区块标题、空状态、信息摘要、品牌说明块），也优先沉淀到 `components/ui/` 下统一复用，页面与业务组件尽量只保留业务编排。
- 顶部消息提示统一使用全站 `Toaster`，展示位置为**页面顶部居中下落式**，危险类消息继续沿用红色提示方案。

## 14. `docs/plan.md` 维护规则

- `docs/plan.md` 是项目中用于记录**当前下一步建议任务**的专用文件。
- Codex 在完成当下任务后，需要先向用户说明建议的下一步；只有在用户明确同意后，才覆盖更新 `docs/plan.md`。
- 更新 `docs/plan.md` 时，始终以“当前确认的下一步”为准，不做历史追加。
- 写入 `docs/plan.md` 与其他中文文档时，必须保证中文可读，不允许出现乱码。
- 对中文文档和中文注释文件进行新增或修改时，默认补齐或保持 **UTF-8 with BOM**，避免 Windows 控制台读取时出现乱码。
